...
{
    for (int i = 0; i < *dim; ++i) {
        if ((*a_sensors)[i].sensor_type == 0) {
            tire_sensor *tire = (tire_sensor *)(*a_sensors)[i].sensor_data;
            if (!tire_check(tire)) {
                remove_sensor(a_sensors, dim, i);
                --i;
            }

        } else if ((*a_sensors)[i].sensor_type == 1) {
            power_management_unit *pmu = (power_management_unit *)(*a_sensors)[i].sensor_data;
            if (!pmu_check(pmu)) {
                remove_sensor(a_sensors, dim, i);
                --i;
            }
